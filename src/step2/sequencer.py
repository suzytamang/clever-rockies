    ...
    def extract(self, main_terms, context_terms, size_context, headers):
        ...
        for term in main_terms:
            ...
            while True:
                ...
                if hit == -1:
                    break
                if (self.text[hit + len(term.label)] not in END_TOKEN) or (
                    self.text[hit - 1] not in END_TOKEN
                ):
                    #    # if hit is PUNCT then do nothing (add it to context), else check both sides of term
                    #    if term._class != 'DOT':
...
if __name__ == "__main__":
    ...
    if len(main_terms) == 0:
        sys.stderr.write(f"Main targets not found ({args.main_targets}) - exiting\n")
        sys.exit(-1)
